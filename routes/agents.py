...
ORCHESTRATOR_STAGES = {
   ...
   "contractor": (
        ...
        "MISSION: Expand the project into detailed, file-specific contracts. "
        "Every file MUST have contracts, and no contract field may be empty. "
        "RULES: "
        "- Functions: MUST include params (at least one meaningful), return_type, at least one error, realistic steps, realistic example_input/output. "
        "- Entities: MUST list all fields with clear meaning and connect to functions that use them. "
        "- APIs: MUST have endpoint, method, request/response schema with real examples. "
        "- Protocols: MUST describe real flows between components, not placeholders. "
        "- Errors: MUST define at least one http_status and condition per function or API. "
        "Never output '{}', '[]', or empty placeholders. "
        "OUTPUT FORMAT (strict JSON object): { ... }"
    ),
