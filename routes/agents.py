...
ORCHESTRATOR_STAGES = {
    ...
    "contractor": (
        ...
        "MISSION: You are the master architect of contracts. Expand the project into the most detailed, "
        "file-specific contracts possible. You must be the best in existence: thorough, precise, realistic, "
        "and fully interconnected. Nothing may be left empty or vague. "
        "RULES (ABSOLUTE): "
        "- Every file from the scoper MUST appear in at least one contract entry. "
        "- NEVER output file names as keys. Always use arrays: entities, apis, functions, protocols, errors. "
        "- Every item MUST have an 'implements' array that lists the file(s) where it belongs. "
        "- Functions: "
        "   • Must always have non-empty 'params' with meaningful names and types. "
        "   • Must always declare at least one error with http_status and condition. "
        "   • Must include detailed 'steps' (>=3) that explain exactly how it works. "
        "   • Must include realistic example_input and example_output (actual JSX, JSON, or object samples). "
        "   • Must reference Entities if relevant. "
        "- Entities: "
        "   • Must have at least 3 fields with types and clear descriptions. "
        "   • Must be explicitly linked to functions and/or APIs that use them. "
        "   • Must show how they are consumed or returned. "
        "- APIs: "
        "   • Must include endpoint, HTTP method, request_schema, response_schema, example_request, example_response. "
        "   • Must always map to at least one function and one entity. "
        "   • Must declare at least one error with http_status. "
        "- Protocols: "
        "   • Must describe real flows between components (user actions, data fetching, rendering). "
        "   • Must have at least 3 steps, no placeholders. "
        "- Errors: "
        "   • Must NEVER be an empty array. "
        "   • Must describe real-world failure cases, mapped to http_status. "
        "- Coverage: "
        "   • Every file must be represented in functions/entities/apis/protocols/errors. "
        "   • No file may be left without contracts. "
        "- Consistency: "
        "   • Names of entities, functions, and APIs must be consistent across contracts. "
        "   • Entities referenced in APIs and functions must match exactly. "
        "OUTPUT: Output ONLY valid JSON, in this strict format (no markdown, no commentary): "
        "{ "
        "  \"entities\": [ "
        ...
        "  ], "
        "  \"protocols\": [ "
        "    { \"name\": \"<ProtocolName>\", \"flow\": [\"Step 1: ...\", \"Step 2: ...\", \"Step 3: ...\"], "
        "      \"implements\": [\"<FileName>\"] } "
        "  ], "
        "  \"errors\": [ "
